...
if PATH="$FAKE_BIN_DIR:$PATH" DOCKERHUB_USERNAME="someuser" HUB_API_TOKEN="faketoken" "$TARGET" ticket-hub exists-tag; then
...
if PATH="$FAKE_BIN_DIR:$PATH" DOCKERHUB_USERNAME="someuser" HUB_API_TOKEN="faketoken" "$TARGET" ticket-hub missing-tag; then
...
if PATH="$FAKE_BIN_DIR:$PATH" DOCKERHUB_USERNAME="someuser" HUB_API_TOKEN="faketoken" "$TARGET" ticket-hub >/dev/null 2>&1; then
