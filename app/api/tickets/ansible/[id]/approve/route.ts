...
/** `id` here is the bare integer -- forwards to PATCH /tickets/ansible/:id/approve on the backend, the ANSIBLE-only approve endpoint. */
export async function PATCH(_request: Request, context: RouteContext) {
  ...
  const apiResponse = await approveAnsibleTicketInBackend(apiUrl, token, id);
  if (!apiResponse) {
...
async function approveAnsibleTicketInBackend(
  apiUrl: string,
  token: string,
  id: string,
): Promise<Response | null> {
  try {
    return await fetch(`${apiUrl}/tickets/ansible/${id}/approve`, {
      method: "PATCH",
      headers: { Authorization: `Bearer ${token}` },
      signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS),
    });
  } catch (error) {
    console.error("Failed to reach ticket-hub-api to approve an ansible ticket", error);
    return null;
  }
}
