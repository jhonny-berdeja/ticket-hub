...
/** `number` here is the bare integer, no prefix -- forwards to GET /tickets/ansible/by-number/:number on the backend, the ANSIBLE-only lookup endpoint. */
export async function GET(_request: Request, context: RouteContext) {
  ...
  const apiResponse = await findAnsibleTicketByNumberInBackend(apiUrl, token, number);
  if (!apiResponse) {
...
async function findAnsibleTicketByNumberInBackend(
  apiUrl: string,
  token: string,
  number: string,
): Promise<Response | null> {
  try {
    return await fetch(`${apiUrl}/tickets/ansible/by-number/${number}`, {
      headers: { Authorization: `Bearer ${token}` },
      signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS),
    });
  } catch (error) {
    console.error("Failed to reach ticket-hub-api to find an ansible ticket by number", error);
    return null;
  }
}
