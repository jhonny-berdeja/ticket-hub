...
export async function POST(request: NextRequest) {
  ...
  const apiResponse = await createTicketInBackend(apiUrl, token, body);
  if (!apiResponse) {
    return NextResponse.json(BACKEND_UNREACHABLE_MESSAGE, SERVICE_UNAVAILABLE_STATUS);
  }
...
async function createTicketInBackend(
  apiUrl: string,
  token: string,
  body: unknown,
): Promise<Response | null> {
  try {
    ...
    console.error("Failed to reach ticket-hub-api to create a ticket", error);
    return null;
  }
}
