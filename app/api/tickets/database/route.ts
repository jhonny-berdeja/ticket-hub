...
/** Forwards to GET /tickets/database on the backend — the DATABASE-only list endpoint. */
export async function GET() {
  ...
  const apiResponse = await listDatabaseTicketsFromBackend(apiUrl, token);
  if (!apiResponse) {
...
async function listDatabaseTicketsFromBackend(
  apiUrl: string,
  token: string,
  ...
  try {
    ...
    console.error("Failed to reach ticket-hub-api for the database ticket list", error);
    return null;
  }
}
