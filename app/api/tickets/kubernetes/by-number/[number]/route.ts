...
/** `number` here is the bare integer, no prefix -- forwards to GET /tickets/kubernetes/by-number/:number on the backend, the KUBERNETES-only lookup endpoint. */
export async function GET(_request: Request, context: RouteContext) {
  ...
  const apiResponse = await findKubernetesTicketByNumberInBackend(apiUrl, token, number);
  if (!apiResponse) {
...
async function findKubernetesTicketByNumberInBackend(
  apiUrl: string,
  token: string,
  number: string,
): Promise<Response | null> {
  try {
    return await fetch(`${apiUrl}/tickets/kubernetes/by-number/${number}`, {
      headers: { Authorization: `Bearer ${token}` },
      signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS),
    });
  } catch (error) {
    console.error("Failed to reach ticket-hub-api to find a kubernetes ticket by number", error);
    return null;
  }
}
