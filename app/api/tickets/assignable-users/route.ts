...
export async function GET() {
  ...
  const apiResponse = await fetchAssignableUsersFromBackend(apiUrl, token);
  if (!apiResponse) {
...
async function fetchAssignableUsersFromBackend(
  apiUrl: string,
  token: string,
): Promise<Response | null> {
  try {
    return await fetch(`${apiUrl}/tickets/assignable-users`, {
      headers: { Authorization: `Bearer ${token}` },
      signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS),
    });
  } catch (error) {
    console.error("Failed to reach ticket-hub-api for assignable-users", error);
    return null;
  }
}
