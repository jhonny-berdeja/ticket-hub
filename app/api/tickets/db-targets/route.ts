...
export async function GET() {
  ...
  const apiResponse = await fetchDbTargetsFromBackend(apiUrl, token);
  if (!apiResponse) {
...
async function fetchDbTargetsFromBackend(
  apiUrl: string,
  token: string,
): Promise<Response | null> {
  try {
    return await fetch(`${apiUrl}/tickets/db-targets`, {
      headers: { Authorization: `Bearer ${token}` },
      signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS),
    });
  } catch (error) {
    console.error("Failed to reach ticket-hub-api for db-targets", error);
    return null;
  }
}
