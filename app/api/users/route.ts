...
export async function GET() {
  ...
  const apiResponse = await listUsersFromBackend(apiUrl, token);
  if (!apiResponse) {
...
export async function POST(request: NextRequest) {
  ...
  const apiResponse = await createUserInBackend(apiUrl, token, body);
  if (!apiResponse) {
    return NextResponse.json(BACKEND_UNREACHABLE_MESSAGE, SERVICE_UNAVAILABLE_STATUS);
  }
...
async function parseRequestBody(request: NextRequest): Promise<unknown> {
  try {
    ...
    console.error("Failed to parse users request body", error);
    return null;
  }
}

async function listUsersFromBackend(
  apiUrl: string,
  token: string,
  ...
  try {
    ...
    console.error("Failed to reach ticket-hub-api for the user list", error);
    return null;
  }
}
...
async function createUserInBackend(
  ...
  try {
    ...
    console.error("Failed to reach ticket-hub-api to create a user", error);
    return null;
  }
}
