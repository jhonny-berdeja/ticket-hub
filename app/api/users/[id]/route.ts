...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  const apiResponse = await updateUserInBackend(apiUrl, token, id, body);
  if (!apiResponse) {
...
async function parseRequestBody(request: NextRequest): Promise<unknown> {
  try {
    ...
    console.error("Failed to parse user update request body", error);
    return null;
  }
}
...
async function updateUserInBackend(
  ...
  try {
    ...
    console.error("Failed to reach ticket-hub-api to update a user", error);
    return null;
  }
}
